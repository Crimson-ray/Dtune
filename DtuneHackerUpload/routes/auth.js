...
router.post('/joinParty/:userId', async (req, res) => {
    try {
        ...
        user.invitesActive.push({ user: userId, playlist: { name, songIds, image },playlist1:userPlaylist._id,playlist2:otherPlaylist._id});
        otherUser.invitesActive.push({ user: req.user._id, playlist: { name, songIds, image },playlist1:otherPlaylist._id,playlist2:userPlaylist._id});
...
let Playlist;
   users.forEach(user=>{
    ...
    console.log(Playlist)
    res.render('users/userPlaylist',{playlist:Playlist,trackUrl})
   })
})
...
router.get('/partyPlaylist/:id', async (req,res,err)=>{
    const {id}=req.params
    const userId=req.user._id
    const user= await User.findById(userId)
    const invite =await  user.invitesActive.id(id);
    const playlist=invite.playlist;
    const trackUrl=[]
    const promises=playlist.songIds.map(async (track,i) => {
        const {liked,disliked}= await getUserLikes(req.user._id,track.trackId)
        track.liked=liked
